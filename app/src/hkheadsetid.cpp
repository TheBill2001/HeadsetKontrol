...
quint16 parseId(QAnyStringView idStringView)
{
    ...
    if (!check) {
        qWarning(HK_LOGGING, "Failed to parse device ID string: \"%s\"", qPrintable(idString));
        return 0;
    }

    if (value == 0 || value > std::numeric_limits<quint16>::max()) {
        qWarning(HK_LOGGING, "Parsed ID string is out of range: \"%s\"", qPrintable(idString));
        return 0;
    }
...
std::pair<quint16, quint16> parseIds(QAnyStringView headsetId)
{
    ...
    if (!check1 || !check2) {
        qWarning(HK_LOGGING, "Failed to parse device ID string: \"%s\"", qPrintable(headsetIdString));
        return std::make_pair<quint16, quint16>(0, 0);
    }

    if (vendorId == 0 || vendorId > std::numeric_limits<quint16>::max()) {
        qWarning(HK_LOGGING, "Vendor ID is out of range for device ID string: \"%s\"", qPrintable(headsetIdString));
        return std::make_pair<quint16, quint16>(0, 0);
    }

    if (productId == 0 || productId > std::numeric_limits<quint16>::max()) {
        qWarning(HK_LOGGING, "Product ID is out of range for device ID string: \"%s\"", qPrintable(headsetIdString));
        return std::make_pair<quint16, quint16>(0, 0);
    }
...
{
    return u"0x%1:0x%2"_s.arg(QString::number(vendorId, 16), QString::number(productId, 16));
}
