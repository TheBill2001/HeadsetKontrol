...
HKHeadsetError HKHeadsetPrivate::makeError(const headsetcontrol::DeviceError &error, HKHeadset::Capability capability) const
{
    return {q_ptr,
            ...
            name,
            capability,
            headsetErrorCodeFromInternal(error.code),
            error.message,
            error.details,
            QDateTime::fromStdTimePoint(std::chrono::time_point_cast<std::chrono::milliseconds>(error.timestamp))};
}
...
HKBattery HKHeadsetPrivate::getBattery(QList<HKHeadsetError> &errors) const
{
    return getResult(errors, internalHeadset.getBattery(), HKHeadset::BatteryStatusCapability, [](headsetcontrol::BatteryResult &&result) -> HKBattery {
        return {HKUtils::clamp<quint8, 0, 100>(result.level_percent),
                batteryStatusFromInternal(result.status),
                microphoneStatusFromInternal(result.mic_status),
                std::move(result).voltage_mv,
                std::move(result).time_to_empty_min,
                std::move(result).time_to_full_min};
    });
}
...
HKChatMix HKHeadsetPrivate::getChatMix(QList<HKHeadsetError> &errors) const
{
    return getResult(errors, internalHeadset.getChatmix(), HKHeadset::ChatMixStatusCapability, [](const headsetcontrol::ChatmixResult &result) -> HKChatMix {
        return {HKUtils::clamp<quint8, 0, 128>(result.level),
                HKUtils::clamp<quint8, 0, 100>(result.game_volume_percent),
                HKUtils::clamp<quint8, 0, 100>(result.chat_volume_percent)};
    });
}
